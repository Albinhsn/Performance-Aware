...
static void allocateStringMmap(String *buffer, u64 len) {
  size_t size = len * sizeof(u8);
  buffer->buffer = (u8 *)mmap(NULL, size, PROT_READ | PROT_WRITE,
...
static void handleAllocation(ReadParameters *params, String *buffer) {
  ...
  case AllocationMode_Mmap: {
    allocateStringMmap(buffer, buffer->len);
    break;
  }
  default: {
...
static void handleDeallocation(ReadParameters *params, String *buffer) {
  ...
  case AllocationMode_Mmap: {
    size_t size = buffer->len * sizeof(u8);
...
static void readFRead(RepetitionTester *repetitionTester,
...
static void readRead(RepetitionTester *repetitionTester,
                     ReadParameters *params) {
  while (IsTesting(repetitionTester)) {
...
int main() {
  ...
  for (;;) {
    for (u32 funcIndex = 0; funcIndex < ArrayCount(testFunctions);
         ++funcIndex) {
      for (u32 allocIndex = 0; allocIndex < AllocationMode_Count;
           ++allocIndex) {
        ...
        printf("\n--- %s%s%s ---\n",
               describeAllocationType(params.allocationMode),
               params.allocationMode != AllocationMode_None ? "+" : "",
               testFunc.Name);
